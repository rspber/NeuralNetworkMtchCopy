...
public class Tensor {
	...
	private final double[] data; // data is stored as a 1-d array in memory, with shapes being row-major indexed.
	...
	public Tensor(int[] shape) {
		...
		for( int d : shape ) {
			if( d < 1 ) {
				throw new RuntimeException("Error: dimensions must be > 0!");
			}
		}
	...
	public Tensor(final int[] shape, final double[] data) {
		this.shape = shape;
		this.data = data;
